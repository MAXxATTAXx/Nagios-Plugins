...
Nagios Plugin to check a Logstash pipeline event parsing time in millisenconds

Outputs the average parsing time in milliseconds for an event in a pipeline, from input to output.
...
class CheckLogstashPipelineLatency(RestNagiosPlugin):
    ...
    def parse_json(self, json_data):
        ...
        if pipeline:
            ...
            events_time = self.calculate_time(events['duration_in_millis'], events['out'])

            filters = reduce(self.process_plugins, plugins['filters'], {'time': 0, 'events': 0})
            outputs = reduce(self.process_plugins, plugins['outputs'], {'time': 0, 'events': 0})

            filters_time = self.calculate_time(filters['time'], filters['events'])
            outputs_time = self.calculate_time(outputs['time'], outputs['events'])

            self.msg += 'avg parsing time = {0:.4f}ms'.format(events_time)
            self.check_thresholds(events_time)
            self.msg += ' | event_parsing_time={0:.4f}ms'.format(events_time)
            self.msg += '{}'.format(self.get_perf_thresholds())
            self.msg += ' filter_parsing_time={0:.4f}ms'.format(filters_time)
            self.msg += ' output_parsing_time={0:.4f}ms'.format(outputs_time)
        else:
            self.critical()
