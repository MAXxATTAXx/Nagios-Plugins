...
Outputs the amount of time logstash spent waiting for a worker to process an event.
...
class CheckLogstashEventsWait(RestNagiosPlugin):

    def __init__(self):
        # Python 2.x
        super(CheckLogstashEventsWait, self).__init__()
        # Python 3.x
        # super().__init__()
        self.name = 'Logstash'
    ...
    def add_options(self):
        super(CheckLogstashEventsWait, self).add_options()
        self.add_thresholds(default_warning=75, default_critical=100)

    def process_options(self):
        super(CheckLogstashEventsWait, self).process_options()
        self.validate_thresholds(integer=False, optional=True)

    def parse_json(self, json_data):
        events = json_data['events']
        out_events = events['out']
        time_millis = events['queue_push_duration_in_millis'] / out_events if out_events > 0 else 0
        self.msg = 'Logstash avg wait time = {0:.4f}ms'.format(time_millis)
        self.check_thresholds(time_millis)
        self.msg += ' | event_wait_time={0:.4f}ms'.format(time_millis)
        self.msg += '{}'.format(self.get_perf_thresholds())

if __name__ == '__main__':
    CheckLogstashEventsWait().main()
